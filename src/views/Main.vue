...
import bgVedio from '../assets/img/bg_1.mp4';
import obj_1 from '../assets/img/obj_1.png';
export default {
  name: 'main',
  mixins:[BasePage],
  data() {
  ...
  methods: {    
    canvasSet(){
      ...
      this.cvObj.bgTexture = PIXI.Texture.fromVideo(bgVedio);

      //let video = vedioTexture.baseTexture.source;
      
      this.cvObj.bgTexture.baseTexture.source.setAttribute('autoplay', false);  
      this.cvObj.bgTexture.baseTexture.source.setAttribute('loop', true);      
      this.cvObj.bg = new PIXI.Sprite(this.cvObj.bgTexture);
      ...
      this.canvas.ticker.add(()=> {
        // console.log(video)
        //this.cvObj.bgTexture.baseTexture.source.pause();
      });
    ...
    leave (el, done) {
        ...
        this.leaveMotion = new TimelineLite({onComplete:()=>{
          done(); 
          this.canvas.stop();
...
<style lang="scss">
  .main{
    ...
    .content{
      ...
      //   width:833px;
      //   height:1099px;
      //   background: url('../assets/img/obj_1.png')
      // }
    }
  }
</style>
